...
| `stamp-duty-amount-from-certificate` | **Bug — the executed Telangana deed prints `[ Stamp duty paid (INR) ]` instead of the duty amount.** Seen on `AMPSFTXU5KV` (TG residential, `SIGN_REQUESTED`): staff attached a real INR 100.00 certificate (`stamp_duty_amount = 100.00`), yet the `Statutory (Telangana)` row is a placeholder. Cause: `stampDutyAmount` is a *customer-entered*, `required: false` field in the now-**mandatory** TG section (rental + commercial). The customer cannot know the duty, so leaves it blank, and the compiler renders a blank key/value row as `[ label ]` (the patch comment "no empty amount ever renders" holds only for the gated clause). The draft is rendered before stamping, and intake only prepends the scan, so `StampInfo.dutyAmount` never reaches the body. A typed amount can also contradict the bound certificate. **Proposal drafted:** `openspec/changes/stamp-duty-amount-from-certificate/` — a system-sourced field hidden from capture, a re-render at intake with the certificate amount (guarded fallback, renderer outage = retryable 503, never `STAMP_FAILED`). Not retroactive: already-stamped agreements keep their instrument. | product owner (direct) | 2026-09-15 | High — a placeholder in the statutory section of every Telangana deed; Telangana is the launch state |
| `tg-stamp-duty-counsel-review` | **The Telangana stamp duty rules are charged on UNVERIFIED figures** (`rules/stamp-duty/TG/*.yaml`, `rules/stamp-paper/TG.yaml`). Counsel must confirm, against primary sources: the Art. 31 rates (0.4% vs 0.5% under one year is an open conflict), whether a refundable security deposit enters the consideration (included conservatively), the Government Order and date for the current rates, the INR 50 counterpart duty (G.O.Ms.120/2015), the paise-vs-rupee rounding practice, the 5-paper limit, whether every lease is compulsorily registrable in Telangana (the registration notice every customer now sees), and the under-stamp warning text (`under-stamp-v1`). Then record `counselReview.contentHash` in each file and stop setting `RULES_STAMP_DUTY_ALLOW_UNREVIEWED`. | `state-stamp-duty-quoting` | 2026-09-16 | High -- blocks charging real customers |
| `tos-below-duty-stamp-choice` | TERMS-OF-SERVICE section 7 still says the total is based on "the stamp duty" and that duty automation is "still being built". The shipped rule prices the **chosen stamp value**, and a customer may choose a value below the duty after acknowledging the under-stamping consequence. Section 7 needs counsel wording for both, and `TermsOfService.vue` must be regenerated from it (jurisdiction-eligibility spec). Deliberately not edited by the implementing change: it is published legal text. | `state-stamp-duty-quoting` | 2026-09-16 | High -- the terms describe a different price basis |
| `stamp-certificate-price-reconciliation` | LEGAL-POSTURE step 3's other half: when the certificate staff buy costs **more** than the frozen stamp value we absorb the difference, and when it costs **less** the customer is owed a refund. Intake only refuses a certificate below the paid-for value today. Needs a reconciliation record and a refund path through Razorpay. | `state-stamp-duty-quoting` | 2026-09-16 | High -- the terms promise no second bill, and over-collection is owed back |
| `ka-stamp-duty-and-template` | Karnataka was dropped from `state-stamp-duty-quoting` when it was rewritten for Telangana. It needs its own sourced rule files, stamp paper catalog, worked cases, counsel review, and a Karnataka template layer set (none exists). | `state-stamp-duty-quoting` | 2026-09-16 | Medium |
