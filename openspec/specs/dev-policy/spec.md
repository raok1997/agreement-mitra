...
The engineering policy enforced by the AgreementMitra harness: what every OpenSpec change must include (unit + integration tests, a PII/security checklist), the documented test-pyramid and scanning posture, and the local guards (PII/secret edit guard, Java auto-format) that keep work inside the project's non-negotiable constraints.
