...
Each entry SHALL carry enough non-PII context to act on it: the agreement's **tracking
reference**, the property **city**, the agreement date, how long it has been waiting, and its
payment state where the payment gate is in use. Entries SHALL be ordered so the longest-waiting
work is visible first.
...
SHALL NOT disclose how many orders exist or reveal any agreement's details.

#### Scenario: Outstanding orders are listed

- **WHEN** a STAFF user opens the console
- **THEN** they see every agreement awaiting stamp intake, each with its tracking reference,
  property city, agreement date, and waiting time, longest-waiting first
