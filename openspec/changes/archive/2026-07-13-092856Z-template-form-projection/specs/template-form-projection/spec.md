...
a data contract of ordered `sections`, each with a `title` and an ordered list of `fields`. The
FormSchema SHALL carry the effective template's `dimensions { state, type }`, `templateId`, `version`,
and `contentHash`. Section order and within-section field order SHALL be preserved exactly as the
effective template declares them. A section entry that is a declared field `key` SHALL become a form
field; a section entry that is a clause `id` SHALL be treated as document content and SHALL NOT appear
as a form field. Each form field SHALL be projected from its declared field, carrying `key`, `label`,
a `widget`, the field `type`, `required`, and -- where present -- `default`, `options`, `group`, and
validation metadata.
...
template, projection SHALL produce the same FormSchema on every run. A `showWhen` condition present on
the effective template MAY be carried into the schema as **opaque metadata**, but the projector SHALL
NOT evaluate it and SHALL NOT mark any field hidden or shown based on it.

#### Scenario: Repeat projection is identical

- **WHEN** the projector is called twice for the same effective template
- **THEN** both calls return an equal FormSchema, with no dependence on time, IO, or randomness
...
- **THEN** it derives the schema only from the effective template's fields and sections and takes no
  user-data input
...
keys, labels, widgets, types, defaults, validation bounds, enum options, group labels, and dimensions
-- and SHALL contain **no signer PII, no secrets, and no instance of user data**. The `documents`
module SHALL expose this capability through a single public named-interface API consisting of the
`FormSchema` DTO and a `TemplateFormApi` port (plus the HTTP controller); the `FormProjector` and the
port implementation SHALL remain module-internal, and no cross-module reach-in SHALL be introduced.
...
- **THEN** it contains only template metadata (keys, labels, widgets, types, defaults, validation,
  options, groups, dimensions) and no signer name, address, identity number, one-time code, or secret

#### Scenario: The public surface is only the form DTO and port

- **WHEN** another module or the SPA consumes this capability
- **THEN** it depends only on the public `FormSchema` DTO and `TemplateFormApi` port (over the named
  interface / HTTP), never on the `FormProjector` or any `documents.template` internal type

#### Scenario: The module boundary stays clean

- **WHEN** `ModularityTests` runs after this change
- **THEN** it passes: the new code adds one `documents` named interface (the form API) and no
  disallowed cross-module dependency, and the projector and port implementation stay package-private
